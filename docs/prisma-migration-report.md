...
- **For future high-traffic or large tables**, when adding secondary indexes in production:
  ...
  ```sql
  -- run outside a long transaction, no CONCURRENT inside Prisma’s single migration transaction pattern is ideal;
  -- some teams use prisma migrate (DDL in shadow DB) for schema truth + separate ops playbooks
